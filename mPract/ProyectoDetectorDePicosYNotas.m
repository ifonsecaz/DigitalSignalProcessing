...
for i=1:length(picos)
    ...
    numN(i)=round(locs(picos(i))); %De la posición del pico, tomo 1024 muestras atrás
end
    ...
    for(i=1:length(F0Armonicos)) %Ciclo que repasa las n frecuencias candidatas
        ...
        if(cont>1)
           ...
           while(mn==0 && m<=length(NotasSimultaneas)) %Recorre el vector de notas simultáneas que va guardando las frecuencias únicas
              ...
              if(abs(aux*NotasSimultaneas(m)-F0Armonicos(i))<=15) %Considero +-5Hz de error
                  mn=1;
              end
              m=m+1;
           end
        ...
        if(j>1&&mn==0) %Si se trata de la segunda ventana en adelante
            ...
            while(mn==0 && m<=length(frecsAnt1)) %Verifica que no sea una nota pasada
                   ...
                   if(abs(aux*frecsAnt1(m)-F0Armonicos(i))<=15)
                       ...
                       if(max(rango)>=0.8*f(F0Armonicos(i))||f2(frecsAnt1(m))>f(F0Armonicos(i))) %Verifica la energía comparada a la pasada
                            mn=1;
                       end
                   end
                   m=m+1;
        ...
        if(j>2&&mn==0) %Si se trata de la tercera ventana en adelante
            ...
            while(mn==0 && m<=length(frecsAnt2)) %Verifica que no sea una nota pasada
                   ...
                   if(abs(aux*frecsAnt2(m)-F0Armonicos(i))<=15)
                       ...
                       if(max(rango)>=0.8*f(F0Armonicos(i))||f2(frecsAnt2(m))>f(F0Armonicos(i))) %Verifica la energía comparada a la pasada
                            mn=1;
                       end
                   end
                   m=m+1;
