...
%numN=(length(t)-inicio)/50; %Para el tema 2 tiene 49 notas, consideramos que tienen un espaciado igual
numN=[17,79,139,197,321,444,565,686,808,929,990,1051,1112,1233,1354,1476,1618]; %Para el tema 1 tiene 16 notas
n=1; %Indice auxiliar para reconocer si ya se completó una ventana
...
for j=inicio:length(t) %El ciclo va a partir de que se considera que inició la canción hasta el final de la señal
    ...
    %if(n>=numN) %Theme2: Revisa cuando se acaba una nota
    if(n>=numN(k)) %Theme1
        X(1,k)=w(Indice); %Agrega al vector con las frecuencias de las notas, la frecuencia con la mayor energía encontrada
        k=k+1;
        %n=1; %Quitar comentario para Theme2
        n=n+1; %Quitar comentario para Theme1
        maxA=0;
        Indice=1;
    else
        n=n+1;
