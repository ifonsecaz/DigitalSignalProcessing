...
plot(1:numel(yAux),yAux,'-',locs(i),yAux(locs(i)),'o ')

%Obtención de los n picos más altos
%picos=i(1:16); %Usar para Theme1
picos=i; %Usar para Theme2
...
figure
yaux=yAux(numN(1):length(yAux));
taux=(0:1:length(yaux)-1)/fs;
Dur=max(taux);
plot(taux,yaux)
hold on
tBPM=0:1/fs:Dur; % time vector
...
tempoCom2=0;
for i=20:200
    f=i/60; %frequency of the impulse in Hz
    BPM=zeros(size(tBPM));
    BPM(1:round(fs/f):end)=1;
    BPM=transpose(BPM);
    %figure
    c=BPM.*yaux;
    if i==136
